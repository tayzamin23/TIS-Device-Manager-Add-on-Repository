...
# Correct StaticFiles import (required for FastAPI)
...
async def api_admin_login(form: OAuth2PasswordRequestForm = Depends()):
    ...
    response.set_cookie("session", token, httponly=True, samesite="lax", max_age=TOKEN_EXPIRY)
    return response
...
@app.get("/api/scan")
async def api_scan(
    base_ip: str = config.IP_COM_DEFAULT_BASE,
    start: int = config.IP_COM_SCAN_START,
    end: int = config.IP_COM_SCAN_END,
    ip_com: Optional[str] = None,
    ...
    if not ip_coms:
        return {"found": [], "message": "No IP-COM found"}
    ...
    for ipc in ip_coms:
        ...
        for dev in discovered:
            entry = appliance_builder.build_device_entry(dev)
            if not any(d.get("subnet") == entry["subnet"] and d.get("device_id") == entry["device_id"] for d in devices):
                devices.append(entry)
                added_devices += 1

                new_apps = appliance_builder.auto_create_appliances_for_device(dev)
                for a in new_apps:
    ...
    return {
        "found": found,
        "devices_added": added_devices,
        "appliances_added": added_appliances
    }
...
async def api_create_appliance(appliance: ApplianceIn, user=Depends(get_current_user)):
    project = store.load_project()
    apps = project.get("appliances", [])
    apps.append(appliance.dict())
    project["appliances"] = apps
    store.save_project(project)
    return appliance
...
async def api_publish(host: str, token: str, user=Depends(get_current_user)):
    ...
    created = await ha_publisher.publish_entities(host, token, apps)

    mapping = []

    for ap in apps:
        name = ap.get("name", "")
        # Fix safe entity ID generation
        safe = "".join(ch for ch in name.replace(" ", "_").lower() if (ch.isalnum() or ch == "_"))

        entity_id = f"tis.{safe}"
        device_ref = ap.get("device")
        channel = ap.get("channels", {}).get("channel")
